...
void setup()  {
  ...
  TV.delay(800);
  TV.println("-- Version 1.1 --");       //version
  TV.delay(1800);
#endif
  ...
  TV.select_font(font4x6);
  TV.println("Please select font size:");
  TV.println(" 1 ");
  TV.select_font(font6x8);
  TV.print("2 ");
  ...
  TV.print("4 ");            // \/ maybe && ... ?
  while(c!=1&&c!=2&&c!=3&&c!=4) c = keyboard.read();
  TV.clear_screen();
  if(c==1) TV.select_font(font4x6);
  else if(c==2) TV.select_font(font6x8);
  else if(c==3) TV.select_font(font8x8);
  else TV.select_font(font8x8ext);
}
